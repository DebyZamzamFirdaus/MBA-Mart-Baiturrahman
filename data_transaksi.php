...
if (isset($_POST['simpan'])) {
    ...
    // Eksekusi query menggunakan method db_query dari object $db_object
    $result = $db_object->db_query($sql);

    if (!$result) {
        // Jika query gagal, tangkap pesan kesalahan dari koneksi
        $error_message = $db_object->db_error();
        echo "<script> location.replace('?menu=data_transaksi&pesan_error=Data gagal disimpan: $error_message'); </script>";
    } else {
        // Jika query berhasil
...
<div class="modal fade" id="modaladd" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form method="post" enctype="multipart/form-data" action="">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle"><i class="fas fa-fw fa-plus"></i> Tambah Data Transaksi</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
